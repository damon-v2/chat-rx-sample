...
extension Reactive where Base: UIAlertController {
    ...
    public func show(in vc: UIViewController,
                     buttons: [UIAlertController.AlertButton],
                     textFields: [TextFieldConfiguration?]) -> Single<(Int, [String])> {
    ...
    public func show(in vc: UIViewController,
                     buttons: [UIAlertController.AlertButton]) -> Single<Int> {
    ...
    public func prompt(in vc: UIViewController,
                       defaultValue: String?,
                       closeTitle: String,
                       confirmTitle: String) -> Maybe<String> {
        return show(in: vc,
                    buttons: [.cancel(closeTitle), .default(confirmTitle)],
                    textFields: [{$0.text = defaultValue}])
            .filter { $0.0 == 1 }
            .map { $0.1[0] }
    }
    ...
    public static func show(in vc: UIViewController, title: String?, message: String?, buttons: [UIAlertController.AlertButton], textFields: [TextFieldConfiguration?], preferredStyle: UIAlertController.Style = .alert) -> Single<(Int, [String])> {
        ...
        return alertView.rx.show(in: vc,
                                 buttons: buttons,
    ...
    public static func show(in vc: UIViewController,
                            ...
                            message: String?,
                            buttons: [UIAlertController.AlertButton],
                            preferredStyle: UIAlertController.Style = .alert) -> Single<Int> {
        return show(in: vc,
                    title: title,
                    message: message,
                    buttons: buttons,
    ...
    public static func show(in vc: UIViewController,
        ...
        return show(in: vc,
                    ...
                    buttons: buttons,
                    textFields: [],
                    preferredStyle: preferredStyle).map { $0.0 }
    }
    ...
    public static func show(in vc: UIViewController, title: String?, message: String?, closeTitle: String) -> Single<Void> {
        return show(in: vc,
                    title: title,
                    message: message,
                    buttons: [.cancel(closeTitle)],
                    textFields: []).map { _ in return }
    }
...
extension UIAlertController.AlertButton {
    var isCancel: Bool {
        switch self {
    ...
    var isNotCancel: Bool { !self.isCancel }
}
